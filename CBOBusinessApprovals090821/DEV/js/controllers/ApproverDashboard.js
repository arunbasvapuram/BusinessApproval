...
function createInnerGrids(objItems, uniqueNames){	
	...
	for(var i=0;i < uniqueNames.length;i++){ 
		...
		var fullresult =  '<table id="table" style="width:100%"><thead><tr><td><b>Staff</b></td>' + '<td><b>Capability/Talent Group/Offering</b></td>'+ '<td><b>Anticipated Completion Date</b></td>' + '<td><b>Project Manager/Coach Name</b></td>' + '<td colspan="2"><b>Action</b></td>' + '</tr></thead><tbody>';
		...
		{  
			...
			fullresult += '<td style="text-align:center;">' + temp2[0][x].Certification_x0020_Title.Capability_x0020_Name + '/' + temp2[0][x].Certification_x0020_Title.Talent_x0020_Group_x0020_Name + '/' + temp2[0][x].Certification_x0020_Title.Title +'</td>';
...
function createPMPendingReqGrid(objItems){	
	...
	var fullresult =  '<table id="table" style="width:100%"><thead><tr><td><b>Staff</b></td><td colspan="2"><b>Certification Name</b></td>' + '<td><b>Capability/Talent Group/Offering</b></td>'+ '<td><b>Anticipated Completion Date</b></td>' +'<td colspan="2"><b>Action</b></td>' + '</tr></thead><tbody>';
	for (var x = 0; x < objItems.length; x++) 
	{  
		...
		fullresult += '<td style="text-align:center;">' + objItems[x].Certification_x0020_Title.Capability_x0020_Name + '/' + objItems[x].Certification_x0020_Title.Talent_x0020_Group_x0020_Name + '/' + objItems[x].Certification_x0020_Title.Title +'</td>';
			...
			if(grpTitle.includes('CBO Certifications Learning Champions')){
				...
				if(approvalStatus == 'Rejected'){
					if(approverComments.trim() == ''){
						...
						return;
					}else if(approverComments.length < 30){
						alert("Minimum 30 char justification is required to Reject.");
						return;
					}
...
function onQuerySucceeded(certId, approvalStatus) {
	...
	getPMCertRequests();
	//location.reload();
	//windows.location.href = "https://americas.internal.deloitteonline.com/sites/USICBOPortal/UsiCboBusinessApprovals/Pages/Dashboard.aspx?team=1";
}
