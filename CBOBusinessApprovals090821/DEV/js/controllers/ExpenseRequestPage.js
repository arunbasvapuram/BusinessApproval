...
function checkForThousand(){
    if($('#EstimatedExpenseAmount').val() < 1000)
    {
        bootbox.alert("Expense amount less than INR 1000, can be directly applied in DTE and does not require approval via this application.");
...
function loadJSSet() {
    if (jsFilePaths.length > 0) {
        ...
        var message = "";
        var table ="";
        $('#requestTypeidentifier').text("New Expense Pre-Approval Request");
        header = "Please go through below guidelines before you apply for any Expense request";
        message = "<br><ul style='line-height:3rem'><li>Please do not use this application to submit expenses that are to be channeled via ARIBA</li><li>For any Project Billable expenses please reach out to your Project Leadership and do not submit the request via this application</li><li>Expense amount less than INR 1000, can be directly applied in DTE and does not require approval via this application</li><li>Expenses other than project expenses, firm approved expenses(ex: BCP expenses) and ARIBA expenses should be channeled via this application for pre-approval</li><li>SM/MD's are not required to raise any Travel Expense request in this application</li><li>Note that this application does not replace DTE. Once the request has been approved in the application, please mention the Request Number in comments while claiming the expense in DTE</li></ul>";
        $('hr').remove();
...
function sortExpenseTypeDropdown() { 
    var option = $("#ExpenseTypeDropDown option[value='0']"); 
    option.appendTo("#ExpenseTypeDropDown"); 
    var options = $("#ExpenseTypeDropDown option:not([value='0'])"); 
    options.detach().sort(function(a, b) { 
        var at = $(a).text(); 
        var bt = $(b).text(); 
        return (at > bt) ? 1 : ((at < bt) ? -1 : 0); 
    }); 
    options.appendTo("#ExpenseTypeDropDown"); 
} 
                                ...
                                if (allData.length > 0) {
                                    $(allData).each(function (index, item) {
                                        if (item.Id == requestId) {
                                            ...
                                            if(item.ThreadorInitiativeLeadId != null){
