...
function fetchDemandDetails(_Id) {
  var DemandListURL =  "/_api/web/lists/GetByTitle('OrderBoardDemand')/items("+_Id+")";
   console.log('URL:',_spPageContextInfo.webAbsoluteUrl + DemandListURL)
  var objHeaders = {
    type: "GET",
    headers: {
  ...
  fetch(_spPageContextInfo.webAbsoluteUrl + DemandListURL, objHeaders)
    ...
    .catch(function (ex) {
      $(".loading").hide();
      $("#dashboardHeader").text("Add Demand Details");
      console.log("fetchDemandDetails error " + ex.message);
    });
}
//#endregion
...
function DemandPeriodValidation(demandPeriod){
  ...
  if(!(parseInt(currentPeriod.substring(1,3))-originalPeriod>1)){swal("Invalid Demand Period", "Previous period data cannot be changed", "error");return false}
