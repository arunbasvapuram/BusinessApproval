...
var ProjectRefusalPortalUrl = "https://americas.internal.deloitteonline.com/sites/USICBOPortal/ProjectRefusal";
var currentUserId = 0;
...
var trackLeavesQueryDataUrl = siteUrl + "/_api/web/lists/getbytitle('EmployeeRequests')/items?$select=*,Practitioner/Title,Practitioner/EMail,Practitioner/JobTitle,OfferingLead/EMail,TalentGroupLead/EMail&$expand=Practitioner/Id,OfferingLead/Id,TalentGroupLead/Id";
function pendingLeavesRequests() {
    if (isAdminTemplate) {
        //Approver
        var adminFilters = [];
        $(userApprovalFor).each(function (idx, val) {
            ...
            if (pendingRole != "TLA") {
                switch (pendingRole) {
                    case "TL":
                        adminFilters.push("(PendingWith eq '" + pendingRole + "' and Offering eq '" + val.offering + "' and TalentTeamPerson/Id eq '" + currentUserId + "')")
                        break;
                    case "TGL":
                        adminFilters.push("(PendingWith eq '" + pendingRole + "' and Offering eq '" + val.offering + "' and TalentGroupLead/Id eq '" + currentUserId + "')");
                        break;
                    case "OL":
                        adminFilters.push("(PendingWith eq '" + pendingRole + "' and Offering eq '" + val.offering + "' and OfferingLead/Id eq '" + currentUserId + "')");
                        break;
                    case "OPL":
                        adminFilters.push("(PendingWith eq '" + pendingRole + "' and Offering eq '" + val.offering + "' and PortfolioLead/Id eq '" + currentUserId + "')")
                        break;
                }
            }
            else if (pendingRole == "TLA") {
                pendingRole = "TL";
                adminFilters.push("(PendingWith eq '" + pendingRole + "' and Offering eq '" + val.offering + "')");
            }
            if (adminFilters.length == 0) {
                adminFilters.push("(PendingWith eq '" + pendingRole + "')");
            }
        });
        ...
        trackLeavesQueryDataUrl = trackLeavesQueryDataUrl + "&$filter=((" + adminFilters.join(" or ") + ") and (Title ne 'Draft'  )) ";
    }
    else {
        //Normal Employee        
        var filterCond = [];
        filterCond.push("Practitioner/EMail eq '" + currentLoggedInUserEmail + "'");
        trackLeavesQueryDataUrl = trackLeavesQueryDataUrl + "&$filter=(Practitioner/EMail eq '" + currentLoggedInUserEmail + "')"
    }
};


var trackExpenseQueryDataUrl = siteUrl + "/_api/web/lists/getbytitle('ExpenseRequestsData')/items?$select=*,Practitioner/Title,Practitioner/EMail,Practitioner/JobTitle,OfferingLead/EMail,TalentGroupLead/EMail&$expand=Practitioner/Id,OfferingLead/Id,TalentGroupLead/Id";
function pendingExpenseRequests() {
    if (isAdminTemplate) {
        ...
        trackExpenseQueryDataUrl = trackExpenseQueryDataUrl + "&$filter=((" + adminFilters.join(" or ") + ") and (Title ne 'Draft'  )) ";
    }
    else {
        //Normal Employee        
        var filterCond = [];
        filterCond.push("Practitioner/EMail eq '" + currentLoggedInUserEmail + "'");
        trackExpenseQueryDataUrl = trackExpenseQueryDataUrl + "&$filter=(Practitioner/EMail eq '" + currentLoggedInUserEmail + "')"
    }
};
        ...
        success: function (transferdata) {
            ...
            loadListItems({
                ...
                success: function (visadata) {
                    loadListItems({
                        url: trackLeavesQueryDataUrl,
                        success: function (leavesData) {
                            loadListItems({
                                url: trackExpenseQueryDataUrl,
                                success: function (expenseData) {
                                    ...
                                    $(leavesData).each(function (idx, val) {
                                        var tempData = val;
                                        tempData.RequestCategory = "Leaves";
                                        tempData.transferFromAndTo = "";
                                        tempData.TypeOfRequest = leaveTypes[val.RequestTypeId] == undefined ? val.RequestTypeId : leaveTypes[val.RequestTypeId];
                                        tempData.practitionerDetails0 = {};
                                        tempData.practitionerDetails0.Title = val.Practitioner.Title;
                                        tempData.practitionerDetails0.JobTitle = val.Practitioner.JobTitle;
                                        tempData.Pending_x0020_With = val.PendingWith;
                                        tempData.ExceptionFlag = null;
                                        totalData.push(tempData);
                                    });
                                    $(expenseData).each(function (idx, val) {
                                        var tempData = val;
                                        tempData.RequestCategory = val.RequestCategory;
                                        tempData.transferFromAndTo = "";
                                        tempData.TypeOfRequest = expenseTypes[val.RequestType] == undefined ? val.RequestType : expenseTypes[val.RequestType];
                                        tempData.practitionerDetails0 = {};
                                        tempData.practitionerDetails0.Title = val.Practitioner.Title;
                                        tempData.practitionerDetails0.JobTitle = val.Practitioner.JobTitle;
                                        tempData.practitionerDetails0.EMail = val.Practitioner.EMail;
                                        tempData.Pending_x0020_With = val.PendingWith;
                                        tempData.ExceptionFlag = null;
                                        totalData.push(tempData);
                                    });
...
function setuserRoleHighest() {
	...
	if(userRoleHighest !== "RM"){
		$('#projectRefusalMenu').hide();
	}
...
function filterRefusalListBasedOnRole(RefusalListDetails) {
	if(userRolesOffering.length > 0){
		$('.projectRefusalSection').show();
	}
	else{
		$('.projectRefusalSection').hide();	
	}
	
    var statusCodesForUserRoles = [];
    var hasRMRoleForMyDashboard = !isAdminTemplate && userRoleHighest === "RM";
    var hasRMRoleForApprovalDashboard = isAdminTemplate && userRoleHighest === "RM";
    var hasTGLRoleForApprovalDashboard = isAdminTemplate && userRoleHighest === "TGL";	
    if (isAdminTemplate) {
        //Approval Dashboard
        var apprvalDashboardRole = userRoleHighest;
        switch (apprvalDashboardRole) {
            case "TL":
                //Or TBA Role
        ...
        //My Dashboard
        var myDashboardRole = userRoleHighest;
        switch (myDashboardRole) {
            case "TL":
                //Or TBA Role
    ...
    if (hasRMRoleForMyDashboard) {
        //Do Nothing Return all rows for RM only View
    }
    else if (hasTGLRoleForApprovalDashboard || hasRMRoleForApprovalDashboard) {
        RefusalListDetails = [];
        $('.projectRefusalSection').hide();
    }
...
function getPractitionerNoOfRefusalsBasedOnRole(fullRefusalListDetails, practitionerId) {
    var NoOfRefusals = 0;
    if (isAdminTemplate) {
        //Approval Dashboard
        if (userRoleHighest === "TL") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId && item.Status === "RTBA"; }).length;
        }
        else if (userRoleHighest === "ES") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId && item.Status === "RES"; }).length;
        }
    }
    else {
        //My Dashboard
        if (userRoleHighest === "RM") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId; }).length;
        }
        else if (userRoleHighest === "TL") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId && item.Status === "RTGL"; }).length;
        }
        else if (userRoleHighest === "ES") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId && (item.Status === "RTBA" || item.Status === "RTGL"); }).length;
        }
        else if (userRoleHighest === "TGL") {
            NoOfRefusals = fullRefusalListDetails.filter(function (item) { return item.PractitionerId === practitionerId && item.Status === "RTGL"; }).length;
        }
    }
    return NoOfRefusals;
...
function getViewURLForProjectRefusal(ele) {
    ...
    if (dataRow && dataRow.PractitionerId && userRoleHighest) {
        if (userRoleHighest === "TL") {
            location.href = ProjectRefusalPortalUrl + "/Pages/TBAView.aspx?PractitionerId=" + dataRow.PractitionerId;
        }
        else if (userRoleHighest === "ES") {
            location.href = ProjectRefusalPortalUrl + "/Pages/ESView.aspx?PractitionerId=" + dataRow.PractitionerId;
        }
        else if (userRoleHighest === "RM") {
            location.href = ProjectRefusalPortalUrl + "/Pages/RMView.aspx?PractitionerId=" + dataRow.PractitionerId;
        }
...
function projectRefusalRequests(PractitionerDetails) {
    ...
    var trackQueryDataUrlRefusalList = ProjectRefusalPortalUrl + "/_api/web/lists/getbytitle('RefusalList')/items?$select=*,ProjectName,PractitionerEmailId";
    //&$filter=PractitionerEmailId eq 'subdeb@Deloitte.com'
    trackQueryDataUrlRefusalList = trackQueryDataUrlRefusalList + "&$filter=(IsClosed eq 0 and IsDeleted eq 0)";
    loadListItems({
        ...
        success: function (data) {
            ...
            $.each(PractitionerDetails, function (index, value) {
                ...
                if (PractitionerRefusals.length > 0) {
                    var NoOfRefusals = getPractitionerNoOfRefusalsBasedOnRole(fullRefusalListDetails, practitionerId);
                    ...
                    buttons: [
                        {
                            extend: 'excel',
                            exportOptions: {
                                columns: $.merge([0, 1, 2, 3, 4], [])//Allowing Exception Flag only for approvers during export
                            },
                    ...
                    "columns": [
                        ...
                        },
                        {
                            ...
                            render: function (data, type, row) {
                                return '<div style="text-align:center">' + row.NoOfRefusals + '</div>';
                            }
                        },
                        {
                            "data": "PractitionerId",
                            ...
                            "width": "8%",
                            "orderable": false,
                            visible: userRoleHighest !== "TGL",
                            render: function (data, type, row) {
                                if (userRoleHighest !== "TGL") {
                                    ...
                                    return '<div style="text-align:center;" class="actionbuttons">' + buttonsTemplate + '</div>';
                                }
                                else {
                                    return "";
                                }
...
function fetchPractitionerDetail() {
    var PractitionerDetailsQuery = ProjectRefusalPortalUrl + "/_api/web/lists/getbytitle('PractitionerDetails')/items?$select=*,PractitionerName";
    loadListItems({
...
function editRecord(ele) {
    ...
    else if (data.RequestCategory == "Leaves") {
        window.location.href = siteUrl + "/Pages/TalentRequest.aspx?TRID=" + data.ID;
    }
    else {
        window.location.href = siteUrl + "/Pages/ExpenseRequestPage.aspx?TRID=" + data.ID;
    }
}
...
function viewRecord(ele) {
    ...
    if (data.RequestCategory == "Transfer") {
        if(isAdminTemplate){
            location.href = siteUrl + "/Pages/TransferRequestView.aspx?ADMIN=1&TRID=" + requestId;
        }
        else{
        ...
        if(isAdminTemplate){
            location.href = siteUrl + "/Pages/VisaRequestView.aspx?ADMIN=1&TRID=" + requestId;
        }
        else{
            location.href = siteUrl + "/Pages/VisaRequestView.aspx?TRID=" + requestId;
        }
        ...
        if(isAdminTemplate){
            location.href = siteUrl + "/Pages/TalentRequestView.aspx?ADMIN=1&TRID=" + requestId;
        }
        else{
            location.href = siteUrl + "/Pages/TalentRequestView.aspx?TRID=" + requestId;
        }
    }
    else if (data.RequestCategory == "Expenses") {
        if(isAdminTemplate){
            location.href = siteUrl + "/Pages/ExpenseRequestView.aspx?ADMIN=1&TRID=" + requestId;
        }
        else{
            location.href = siteUrl + "/Pages/ExpenseRequestView.aspx?TRID=" + requestId;
        }
    }
}
...
function approveRecord(ele) {
    ...
    if (data.RequestCategory == "Transfer") {
        ...
        location.href = siteUrl + "/Pages/TalentApprovalPage.aspx?TRID=" + requestId;
    }
    else {
        location.href = siteUrl + "/Pages/ExpenseApprovalPage.aspx?TRID=" + requestId;
    }
}
            ...
            return {
                ...
                "columns": [
                    ...
                    },
                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            else if(row.RequestCategory == "Expenses"){
                                status = "Req# : " + row.ID + 
                                // "</br>  Expense Type : " + row.TypeOfRequest + 
                                "</br>  Expense Amount : " + row.EstimatedExpenseAmount + 
                                "</br>  Practitioner�s Email ID : " + row.practitionerDetails0.EMail;
                            }
                ...
                "createdRow": function (row, data, dataIndex) {
                    if (isAdminTemplate && data.ExceptionFlag == 'Y') {
                        ...
                        $(row).find('td').first().addClass('sorting_1');
...
function checkIfUserIsApproval() {
	...
	if(!isAdminTemplate){
        ...
        checkPMToShowCeertApproval();
        $("#tblCertificationRequest").DataTable();
	}
...
//check PM To Show Certification Approval Tab
function checkPMToShowCeertApproval(){
    var loggedinUserEmail = currentUser.get_email();

    $.ajax({
        url: "https://americas.internal.deloitteonline.com/sites/USICBOPortal/BusinessApproval/_api/web/lists/getbytitle('Certification Requests')/items?$select=*,Project_x0020_Manager/EMail&$expand=Project_x0020_Manager&$filter=PM_x0020_Approval_x0020_Status eq 'Pending'",
        ...
        success : function(data){
            ...
            for(i = 0; i< objItems.length; i++){    
                if(uniqueNames.indexOf(objItems[i].Project_x0020_Manager.EMail) === -1){
                    uniqueNames.push(objItems[i].Project_x0020_Manager.EMail);        
                }   
            ...
            if(uniqueNames.indexOf(loggedinUserEmail) != -1){
                console.log("present");
                $('#certPMApprovalsBlock').show();
            }
...
function getCertificationRequestItems() {
    ...
    return $.ajax({
        url: "https://americas.internal.deloitteonline.com/sites/USICBOPortal/BusinessApproval/_api/web/lists/getbytitle('Certification Requests')" +
            ...
            "Requestor_x0020_Name&$filter=Requestor_x0020_Name/Id eq '" + loggedInUserID + "'&$orderby=Created desc",
        contentType: "application/json;odata=verbose",
        type: 'GET',
        async: false,
        headers: {
    ...
    getCertificationRequestItems().done(function (data) {
        ...
        for (i = 0; i < objItems.length; i++) {
            ...
            var itemCertificationReq = {
                ...
                PMApprovalStatus: objItems[i].PM_x0020_Approval_x0020_Status != null ? objItems[i].PM_x0020_Approval_x0020_Status : '',
            }
            ...
            $('#tblCertificationRequest tbody').append('<tr style="text-align:center;"><td style="padding:10px;">' +
                ...
                '</td><td style="padding:10px;">' + itemCertificationReq.PMApprovalStatus +
                ...
                '</td><td style="padding:10px;"><a href="https://americas.internal.deloitteonline.com/sites/USICBOPortal/BusinessApproval/Pages/CertificationRequest.aspx?RequestId=' + itemCertificationReq.RowId+'" name = "' + itemCertificationReq.RowId + '" id = "view_' + itemCertificationReq.RowId + '" target="_blank">View</a></td></tr>');            
        }
    });    
}


// certification requests End
